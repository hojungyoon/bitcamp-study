...
- 학습 목표 달성 확인 목록
    ...
    - [] 암시적 형변환을 설명할 수 있는가?
    정수와 소수타입에서만 암시적 형변환이 불가능 하다.
    byte+byte, short+short의 계산은 불가능하다. 그래서 작은 크기의 변수타입, bit와 byte는 연산시에 정수의 기본형인 int타입으로 자동 캐스팅이 된다.
    정수+소수의 경우는 소수(float,double)로 자동 형변환이 이루어진다.

    - [] 명시적 형변환을 사용할 수 있는가?
    int형 변수 두 값이 존재할 때, 부동소수점 타입의 변수에서 연산을 한다 하더라도, 값은 int값이 나오게 된다. 따라서 두 변수에는 임의적으로 캐스팅을 해 주어야 소수점 값을 얻을 수 있다.

    - [] 부동소수점의 값을 비교할 때 극소수의 값을 고려해야 함을 이해하는가?
    직접 입력한 수와 연산을 통해 얻은 값은 다른 수 이다. 왜냐하면, IEEE754 방식으로 연산하는 방식은 근사치를 제공할 뿐 정확한 값이 아니다.(표현가능한 자릿수 끝에 남는 수가 붙는다.)
    그래서 비교연산자를 통한 방법으로는 비교가 불가능 하다.
    ...
    "=="연산자를 사용하여 연산하면 같은 값처럼 보일지라도, false가 나온다.

    - [] 논리 연산자(&&,||,!,^,&,|)를 다룰 수 있는가?
    - [] &&와 &, ||와 | 논리 연산자의 차이점을 아는가?
    &&와 &의 차이점
    &&은 왼쪽값이 정해지면, 오른쪽값에 식이 존재하더라도, 실행되지 않고, 왼쪽값으로 값이 정해진다. 하지만, &의 경우는 왼쪽식과 오른쪽식 둘다 실행하여 결과를 알려준다.
    ||와 |의 차이점
    ||도 마찬가지로 왼쪽값을 알면 결과를 알수 있기 때문에, 오른쪽의 식은 실행되지 않는다. |도 마찬가지로 왼쪽값을 알면 결과를 알 수 있지만, 오른쪽값도 실행하여 결과를 리턴한다.
    ...
    << 비트 전체를 왼쪽으로 n만큼 이동하고, 메모리에서 밀려난 숫자는 지우고, 빈곳을 0으로 채운다. 앞의수*2의 값이 나온다.
    >> 비트 전체를 오른쪽으로 n만큼 이동하고, 메모리에서 밀려난 숫자는 지우고, 빈곳을 1로 채운다.
    >>> 비트 전체를 오른쪽으로 n만큼 이동하고, 메모리에서 밀려난 숫자는 지우고, 빈곳을 0으로 채운다. 1칸씩 밀려날때마다, 앞의 수/2가 된다.

    - [] 비트 이동 연산자를 사용하여 2의 배수를 곱하고 나누는 방법을 아는가?
    - [] 조건 연산자(?:)를 다룰 수 있는가?
    (조건식 ? A : B) = 조건식이 true면 A, false면 B를 출력
